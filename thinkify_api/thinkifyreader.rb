...
class ThinkifyReader < SerialConnection
  ...
  def initialize(com=nil, debug_com = false)
    ...
    begin
      send_receive("brs") #This will die if we failed to initialize properly...
      @connected = true
    rescue
      @connected = false
    end
...
public
  ...
  #use the new XS descriptor to read a large hunk of user memory
  def read_user_big(num_pages="1A")

    self.mask_strongest_tag

    #setup to read out the user memory
    execute("xs03#{num_pages}00")

    #read it!
    strTemp =  execute("t61").split("\r\n")
    #p strTemp
    ...
    strTemp.each do |line|
      if line.include?("READ")
        act1 = line.split("=")
        act2 = act1[1].split(" ")
        retvalue = retvalue + act2[0]
      end
    end
  ...
  def program_user_big(data, mask_strongest=true)
    ...
    offset =["00","08","10","18","20","28","30","38","40","48","50","58","60","68","70","78","8100","8108","8110", "8118","8120","8128","8130","8138","8140","8148"]
    ...
    0.upto(num_chunks/4) do |i|

      chunk = i * 4

      str_chunk_0 = data[(chunk * 32)..(chunk * 32 + 31)]
      str_chunk_1 = data[((chunk+1) * 32)..((chunk+1) * 32 + 31)]
      str_chunk_2 = data[((chunk+2) * 32)..((chunk+2) * 32 + 31)]
      str_chunk_3 = data[((chunk+3) * 32)..((chunk+3) * 32 + 31)]
      ...
      if (foo)
        ...
        unless (str_chunk_0.nil?) or (str_chunk_0 =="")
            str_chunk_0 =str_chunk_0.ljust(32,"0")
            p str_chunk_0
            sr=execute("xw033#{str_chunk_0.size/4}#{offset[chunk]}#{str_chunk_0}")
        end
        unless (str_chunk_1.nil?) or (str_chunk_1 =="")
            str_chunk_1 =str_chunk_1.ljust(32,"0")
            p str_chunk_1
            sr=execute("xw133#{str_chunk_1.size/4}#{offset[chunk+1]}#{str_chunk_1}")
        end
        unless (str_chunk_2.nil?) or (str_chunk_2 =="")
            str_chunk_2 =str_chunk_2.ljust(32,"0")
            p str_chunk_2
            sr=execute("xw233#{str_chunk_2.size/4}#{offset[chunk+2]}#{str_chunk_2}")
        end
        unless (str_chunk_3.nil?) or (str_chunk_3 =="")
            str_chunk_3 =str_chunk_3.ljust(32,"0")
            p str_chunk_3
            sr=execute("xw333#{str_chunk_3.size/4}#{offset[chunk+3]}#{str_chunk_3}")
        end
